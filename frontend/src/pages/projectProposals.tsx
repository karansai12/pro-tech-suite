...
const ProjectProposals = () => {
  ...
  return (
   <div>
    {!isManager ?(<Button asChild><Link to="new-proposal">Create New Proposal</Link></Button>):null }
                    ...
                    <TableCell className="text-right">
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              {!isManager ?(<DropdownMenuItem onClick={()=>{navigate(`/proposals/edit/${item.proposalId}`)}} >Edit</DropdownMenuItem>):null}
                                
                                {isManager?( <DropdownMenuItem onClick={()=>{onclickApprove(item)}}>
                                    Approve
                                </DropdownMenuItem>):null }
                                <DropdownMenuSeparator />
                                {isManager?(<DropdownMenuItem variant="destructive" onClick={()=>{onclickReject(item)}}>
                                    Reject
                                </DropdownMenuItem>):null}
                                {!isManager ?( <DropdownMenuItem variant="destructive" onClick={() => onClickDelete(item.proposalId)}> 
                                    Delete
                                </DropdownMenuItem>):null}
