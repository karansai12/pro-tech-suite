...
export const getAllProposals = async (req: CustomeRequest, res: Response) => {
  try {
    ...
    return res
      .status(400)
      .json({ message: "Failed to fetch proposals.", error });
  }
};
...
export const getProposalById = async (req: CustomeRequest, res: Response) => {
  try {
    ...
    return res
      .status(400)
      .json({ message: "Failed to fetch proposal.", error });
  }
};
...
export const createProposal = async (req: CustomeRequest, res: Response) => {
  ...
  try {
    ...
    if (req.user.role !== Role.employee) {
      return res
        .status(400)
        .json({ message: "manager can not creat proposal" });
    }
    ...
    return res
      .status(400)
      .json({ message: "Failed to create proposal.", error });
  }
};
...
export const updateProposal = async (req: CustomeRequest, res: Response) => {
  try {
    ...
    if (req.user?.role === Role.employee && existing.userId !== req.user.id) {
      return res
        .status(403)
        .json({ message: "You can only edit your own proposals." });
    }
    ...
    return res
      .status(200)
      .json({ message: "Proposal updated successfully.", updated });
  } catch (error) {
    return res
      .status(400)
      .json({ message: "Failed to update proposal.", error });
  }
};
...
export const deleteProposal = async (req: CustomeRequest, res: Response) => {
  try {
    ...
    if (
      ...
      existingProposal?.userId !== req.user.id
    ) {
      return res
        .status(403)
        .json({ message: "You can only delete your own proposals." });
    }
    ...
    return res
      .status(400)
      .json({ message: "Failed to delete proposal.", error });
  }
};
